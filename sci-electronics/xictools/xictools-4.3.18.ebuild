...
# Portage.
SRC_URI="https://github.com/wrcad/xictools/archive/refs/tags/xt-4.3.18.tar.gz"

# Source directory; the dir where the sources can be found (automatically
# unpacked) inside ${WORKDIR}.  The default value for S is ${WORKDIR}/${P}
# If you don't need to change it, leave the S= line out of the ebuild
# to keep it tidy.
S="${WORKDIR}/xictools-xt-4.3.18"
...
src_configure() {
	...
	XIC="mrouter xic"
	SUBDIRS="xt_base ${WRSPICE} ${XIC} fastcap fasthenry"

	CFARGS="${PREFIX} ${ITOPOK} ${NOMOZY} ${GFXLOC}"
...
src_compile() {
	...
	WRSPICE="adms wrspice"
	#vl
	XIC="mrouter xic"
	SUBDIRS="${WRSPICE} ${XIC} fastcap fasthenry"
	...
	sed -i 's/v       rm -f Makefile/\trm -f Makefile/g' Makefile
	emake unpack
	emake patch
	emake
	cd ../
...
# 	for myvar in ${SUBDIRS} ; do
# 		einfo "Building ${myvar}"
# 		cd ${myvar}/
# 			emake
# 		cd ../
# 	done
...
src_install() {
	...
	# setting).
	WRSPICE="adms vl wrspice"
	XIC="mrouter xic"
	SUBDIRS="xt_base ${WRSPICE} ${XIC} fastcap fasthenry"

	for myvar in ${SUBDIRS} ; do
	einfo "Installing ${myvar}"
	cd ${myvar}/
		emake \
			-j1 \
			prefix="${D}"/usr \
